...
import 'models/embedding_model.dart' as local_models;
import 'models/model.dart' as local_models;
...
class _UniversalDownloadScreenState extends State<UniversalDownloadScreen> {
  ...
  void _initializeServices() {
    if (widget.model.isEmbeddingModel) {
      _embeddingDownloadService = EmbeddingModelDownloadService(
        model: widget.model as local_models.EmbeddingModel,
      );
    } else {
      final inferenceModel = widget.model as local_models.Model;
          ...
          children: [
            ...
            if (widget.model.isEmbeddingModel) ...[
              _buildInfoRow('Dimension:',
                  '${(widget.model as local_models.EmbeddingModel).dimension}D'),
            ] else ...[
  ...
  void _proceedToNextScreen() async {
    // Model is downloaded and installed; return to caller (settings /
    // model selection). The app's primary flow doesn't open a chat
    // playground after download — the active model is consumed
    // directly by the Study Hub generators.
